import {FIRST,LAST_CLASS,MILESTONES,assessmentsForDate,nextScheduledEvent,dateKey,addDays,weekday,formatDate,scheduleForDate,dateEvent,holiday,summary,civil} from './model.mjs';
...
function tick(){
  ...
  if(next){const inClass=now>=next.startAt;setText('#next-label',next.kind==='deadline'?'Next deadline':inClass?'Happening now':next.kind==='exam'?'Next final exam':'Next on your schedule');setText('#next-day',next.date===today?'Today':formatDate(next.date,{weekday:'short',month:'short',day:'numeric'}));setText('#next-time',`${next.id} · ${next.time}${next.kind==='exam'?' · '+next.location:''}`);}
  else{setText('#next-label','Regular classes');setText('#next-day','All wrapped up');setText('#next-time','All scheduled classes and final assessments are complete.');}
...
function assessmentBlock(a,now){return `<div class="class-block final exam-block${a.endAt<=now?' past':''}"><strong>${a.id}</strong><span>${a.title}</span><span>${a.time}</span><small>${a.location}</small>${a.endAt<=now?'<small>Finished</small>':''}</div>`;}
function renderCalendar(){
  ...
  if(view==='week'){
    ...
    $('#week-grid').innerHTML=Array.from({length:5},(_,i)=>{
      const k=addDays(start,i),sessions=scheduleForDate(k),assessments=assessmentsForDate(k),event=dateEvent(k);
      return `<div class="day-column${today===k?' is-today':''}" data-date="${k}"><h3>${formatDate(k,{weekday:'short'}).toUpperCase()} <strong>${Number(k.slice(-2))}</strong></h3>${sessions.map(s=>classBlock(s,now)).join('')}${assessments.map(a=>assessmentBlock(a,now)).join('')}${!sessions.length&&!assessments.length?`<div class="no-class${event?' break':''}">${event|| (k<FIRST?'Term starts Aug 18':k>LAST_CLASS?'No regular classes':'No classes')}</div>`:event&&!assessments.length?`<div class="schedule-exception">${event}</div>`:''}</div>`;
    }).join('');
  }else{
    ...
    $('#month-grid').innerHTML=['M','T','W','T','F','S','S'].map(v=>`<div class="month-label" aria-hidden="true">${v}</div>`).join('')+Array.from({length:cells},(_,i)=>{
      const k=addDays(gridStart,i),sessions=scheduleForDate(k),assessments=assessmentsForDate(k),event=dateEvent(k);
      const classes=`month-day${k.slice(0,7)!==month?' outside':''}${k===today?' is-today':''}${k===selected?' selected':''}`;
      return `<div class="${classes}"><time datetime="${k}">${Number(k.slice(-2))}</time><div class="day-dots" aria-hidden="true">${sessions.map(s=>`<i class="legend-dot ${s.style}"></i>`).join('')}${assessments.map(()=>'<i class="legend-dot final"></i>').join('')}${holiday(k)?'<i class="legend-dot holiday"></i>':''}</div>${event?`<span class="day-event">${event}</span>`:''}<button tabindex="0" data-date="${k}" aria-label="${formatDate(k,{weekday:'long',month:'long',day:'numeric'})}: ${[...sessions.map(s=>s.id),...assessments.map(a=>a.id+' '+a.title)].join(', ')||'No regular classes'}${event?', '+event:''}" aria-pressed="${k===selected}"></button></div>`;
    }).join('');
    renderDayDetail();
  }
...
function renderDayDetail(){const sessions=scheduleForDate(selected),assessments=assessmentsForDate(selected),event=dateEvent(selected);$('#day-detail').innerHTML=`<strong>${formatDate(selected,{weekday:'long',month:'long',day:'numeric'})}</strong>${sessions.map(s=>`<p>${s.id} · ${s.time}</p>`).join('')||'<p>No regular classes.</p>'}${assessments.map(a=>`<p class="assessment-detail"><strong>${a.id} · ${a.title}</strong><br>${a.time} · ${a.location}${a.section?' · Section '+a.section:''}</p>`).join('')}${event&&!assessments.length?`<p>${event}${event==='Exam day'?' · University exam period; no personal final scheduled today.':''}</p>`:''}`;}
